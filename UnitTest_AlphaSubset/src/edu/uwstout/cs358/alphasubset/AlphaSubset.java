...
public class AlphaSubset {
	...
	public ArrayList<Character> getSubset() throws Exception {
		if (mPhrase == null) {
			throw new Exception("Phrase was invalid");
	...
	public ArrayList<Character> getUniqueSortedSubset() {
		TreeSet<Character> uniqueLetters = new TreeSet<>();
		try {
			if (mPhrase != null) {
				for (char c : mPhrase.toUpperCase().toCharArray()) {
